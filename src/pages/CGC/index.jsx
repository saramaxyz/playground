...
const TestContainer = ({name, description, image, route = null}) => {
    ...
    return <Card className="test-container">
        ...
        <h3>{name}</h3>
        {(typeof route === "string") ?
            <Button onClick={() => navigate(`/training/${route}`,{state: {description,name}})} variant="contained" color="success">
                Test
            </Button> :
    ...
    return <div style={{
        ...
        <GridContainer>
            ...
            <div className={"cgc-container__tests"}>
                {
                    tests.map(({name, description, image,route}) => {
                        return <TestContainer key={name} name={name} description={description} image={image} route={route}/>
                    })
                }
            </div>
        </GridContainer>
